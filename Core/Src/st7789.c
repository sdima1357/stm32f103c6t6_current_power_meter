#include "st7789.h"
//https://www.waveshare.com/product/displays/lcd-oled/lcd-oled-3.htm
/**
...
void ST7789_Init(void)
{
    ...
    ST7789_WriteSmallData (0x15);			//	(50HZ)
...
void ST7789_Fill(uint16_t xSta, uint16_t ySta, uint16_t xEnd, uint16_t yEnd, uint16_t color)
{
	...
	int wh = (xEnd-xSta+1)*(yEnd-ySta+1);
	//HAL_DMA_PollForTransfer(ST7789_SPI_PORT.hdmatx,HAL_DMA_FULL_TRANSFER,1);
	while(!flgCmplt);
	ST7789_SetAddressWindow(xSta, ySta, xEnd, yEnd);

	uint8_t datah = color >> 8;
	uint8_t datal = color &0xff;

	ST7789_Select();
	ST7789_DC_Set();
	int cnmS = wh>BS?BS:wh;
	...
	while(wh>BS)
	{
		while(!flgCmplt);
		flgCmplt = 0;
		HAL_SPI_Transmit_DMA(&ST7789_SPI_PORT, buff, BS*2);
		//while(!flgCmplt);
		//int cnt = 50;
		//while(HAL_DMA_PollForTransfer(ST7789_SPI_PORT.hdmatx,HAL_DMA_FULL_TRANSFER,1)!=0&&cnt){cnt--;}

		//HAL_SPI_Transmit(&ST7789_SPI_PORT, buff, BS*2, HAL_MAX_DELAY);
		wh-=BS;
	}
	//HAL_DMA_PollForTransfer(ST7789_SPI_PORT.hdmatx,HAL_DMA_FULL_TRANSFER,1);
	while(!flgCmplt);
	...
	while(!flgCmplt);
	//while(!flgCmplt);
	//HAL_SPI_Transmit(&ST7789_SPI_PORT, buff, wh*2, HAL_MAX_DELAY);
	ST7789_UnSelect();
}
...
void ST7789_WriteChar3(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor)
{
	uint32_t i, b, j;
	ST7789_Select();
	ST7789_SetAddressWindow(x, y, x + font.width*3 - 1, y + font.height*3 - 1);
...
void ST7789_WriteString2(uint16_t x, uint16_t y, const char *str, FontDef font, uint16_t color, uint16_t bgcolor)
{
	...
	while (*str) {
		...
		ST7789_WriteChar2(x, y, *str, font, color, bgcolor);
		x += font.width*2;
		str++;
	}
	ST7789_UnSelect();
}
void ST7789_WriteString3(uint16_t x, uint16_t y, const char *str, FontDef font, uint16_t color, uint16_t bgcolor)
{
	ST7789_Select();
	while (*str) {
		if (x + font.width*3 >= ST7789_WIDTH) {
			x = 0;
			y += font.height*3;
			if (y + font.height*3 >= ST7789_HEIGHT) {
				break;
			}
		...
		ST7789_WriteChar3(x, y, *str, font, color, bgcolor);
		x += font.width*3;
		str++;
	}
	ST7789_UnSelect();
